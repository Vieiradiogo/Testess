const alojamentos = [
  {
    id: 1,
    nome: "Albergue do Caminho",
    imagem: "./assets/albergue do caminho.webp",
    localizacao: "Porto",
    tipo: "Albergue",
    precoPorNoite: 15,

    coordenadas: { lat: 41.1579, lng: -8.6291 },
    servicos: ["Wi-Fi", "Pequeno-almoço", "Roupa de cama"],
    contacto: {
      telefone: "+351 912 345 678",
      email: "contato@alberguedocaminho.pt"
    }
  },
  {
    id: 2,
    nome: "Casa do Peregrino",
    imagem: "./assets/casa do peregrino.webp",
    localizacao: "Valença",
    tipo: "Guesthouse",
    precoPorNoite: 25,

    coordenadas: { lat: 42.0314, lng: -8.6455 },
    servicos: ["Wi-Fi", "Estacionamento", "Pequeno-almoço", "Toalhas"],
    contacto: {
      telefone: "+351 913 222 111",
      email: "reservas@casadoperegrino.com"
    }
  },
  {
    id: 3,
    nome: "Pousada Santiago",
    imagem: "./assets/pousada santiago.webp",
    localizacao: "Santiago de Compostela",
    tipo: "Hotel",
    precoPorNoite: 60,

    coordenadas: { lat: 42.8805, lng: -8.5457 },
    servicos: ["Wi-Fi", "Pequeno-almoço", "Restaurante", "Lavandaria"],
    contacto: {
      telefone: "+34 981 123 456",
      email: "info@pousadasantiago.es"
    }
  },
  {
    id: 4,
    nome: "Refúgio do Caminhante",
    imagem: "./assets/refugio do caminhante.webp",
    localizacao: "Tui",
    tipo: "Albergue",
    precoPorNoite: 18,

    coordenadas: { lat: 42.0471, lng: -8.6440 },
    servicos: ["Wi-Fi", "Pequeno-almoço"],
    contacto: {
      telefone: "+34 986 654 321",
      email: "refugio@caminhante.es"
    }
  },
  {
    id: 5,
    nome: "Hospedaria Estrela do Norte",
    imagem: "./assets/hospededaria.webp",
    localizacao: "Ponte de Lima",
    tipo: "Albergue",
    precoPorNoite: 22,

    coordenadas: { lat: 41.7677, lng: -8.5836 },
    servicos: ["Wi-Fi", "Toalhas", "Lavandaria"],
    contacto: {
      telefone: "+351 916 789 123",
      email: "geral@estreladonorte.pt"
    }
  },
  {
    id: 6,
    nome: "Quinta do Caminho",
    imagem: "./assets/quinta do caminho.webp",
    localizacao: "Rubiães",
    tipo: "Guesthouse",
    precoPorNoite: 30,

    coordenadas: { lat: 41.8931, lng: -8.6643 },
    servicos: ["Wi-Fi", "Pequeno-almoço", "Estacionamento"],
    contacto: {
      telefone: "+351 914 222 333",
      email: "info@quintadocaminho.pt"
    }
  },
  {
    id: 7,
    nome: "Albergue São Tiago",
    imagem: "./assets/albergue sao tiago.webp",
    localizacao: "Barcelos",
    tipo: "Albergue",
    precoPorNoite: 14,

    coordenadas: { lat: 41.5316, lng: -8.6187 },
    servicos: ["Wi-Fi", "Roupa de cama"],
    contacto: {
      telefone: "+351 917 654 321",
      email: "sao.tiago@albergues.pt"
    }
  },
  {
    id: 8,
    nome: "Hotel Caminho Real",
    imagem: "./assets/hotel caminho real.webp",
    localizacao: "Ourense",
    tipo: "Hotel",
    precoPorNoite: 55,

    coordenadas: { lat: 42.3361, lng: -7.8640 },
    servicos: ["Wi-Fi", "Pequeno-almoço", "Restaurante", "Spa"],
    contacto: {
      telefone: "+34 988 111 222",
      email: "reservas@caminhoreal.es"
    }
  },
  {
    id: 9,
    nome: "Hotel Trilho Verde",
    imagem: "./assets/hostel trilho verde.webp",
    localizacao: "Redondela",
    tipo: "Hotel",
    precoPorNoite: 20,

    coordenadas: { lat: 42.2836, lng: -8.6094 },
    servicos: ["Wi-Fi", "Pequeno-almoço", "Lavandaria"],
    contacto: {
      telefone: "+34 986 777 888",
      email: "trilhoverde@hostel.es"
    }
  },
  {
    id: 10,
    nome: "Estalagem da Ponte",
    imagem: "./assets/estalagem.webp",
    localizacao: "Padrón",
    tipo: "Albergue",
    precoPorNoite: 28,

    coordenadas: { lat: 42.7383, lng: -8.6608 },
    servicos: ["Wi-Fi", "Pequeno-almoço", "Toalhas"],
    contacto: {
      telefone: "+34 981 345 987",
      email: "reservas@estalagemdaponte.es"
    }
  }
];

module.exports = { alojamentos };
